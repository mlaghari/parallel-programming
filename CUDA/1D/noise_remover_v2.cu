...
Compute1(unsigned char *d_image, double std_dev, double *d_north_deriv, double *d_south_deriv, double *d_east_deriv, double *d_west_deriv, double *d_diff_coef, int width, int height, int blocksPerGrid) {
	...
	if (k >= threadsPerRow && k < totalThreads-threadsPerRow) {
		if (((k % threadsPerRow) != 0) && ((k+1) % threadsPerRow) != 0) {
			...
			d_north_deriv[k] = d_image[(k-threadsPerRow)] - d_image_temp;	// north derivative --- 1 doubleing point arithmetic operations
			north_deriv_value = d_north_deriv[k];
			d_south_deriv[k] = d_image[(k+threadsPerRow)] - d_image_temp;	// south derivative --- 1 doubleing point arithmetic operations
			south_deriv_value = d_south_deriv[k];
			d_west_deriv[k] = d_image[k - 1] - d_image_temp;	// west derivative --- 1 doubleing point arithmetic operations
			west_deriv_value = d_west_deriv[k];
			d_east_deriv[k] = d_image[k + 1] - d_image_temp;	// east derivative --- 1 doubleing point arithmetic operations
