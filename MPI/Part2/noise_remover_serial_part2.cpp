		...
		// {
			...
			if (size > 1) {
				...
				for (int w = 1; w < width-1; w++) {
				  lclimage[0*width + w] = lclimage[(height-2)*width + w];
				  lclimage[(height-1)*width + w] = lclimage[1*width + w];
				}
			}
			...
			if (size > 1) {
				...
				for (int w = 1; w < width-1; w++) {
				    lcl_diff_coef[width*(height-1) + w] = lcl_diff_coef[1*width + w];
				}
			}
			// COMPUTE 2
